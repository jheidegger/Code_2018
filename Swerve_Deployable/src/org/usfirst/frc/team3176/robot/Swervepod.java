...
public class Swervepod extends Subsystem {
	...
	public void setPod(double Speed, double Angle){
		...
		speedSet  = Speed * fps2ups * 1.5;// * direction; 
		...
		driveMotor.set(ControlMode.Velocity, -speedSet);
	...
	private double findSteerPosition(double wantedAngle){
		 ...
		 */
		 rawAngle = steerMotor.getSelectedSensorPosition(0) -Constants.OFFSETS[id];
		 ...
		 if (Math.abs(deltaPhi) > (PI/2)) {
//			 if(wantedAngle > -.01 && wantedAngle < .01 && speedSet < 0) {
//				 wantedAngle = .01;
//			 }
		      deltaPhi -= Math.copySign(PI, deltaPhi);
		      
		      speedSet = -speedSet;
		    }
		 steerFinal = (((deltaPhi) /(2*PI))*4096);
		 return (rawAngle + steerFinal + Constants.OFFSETS[id]);
		 /*
	...
	{
		return (steerMotor.getSelectedSensorPosition(0)%4096);
	}
	@Override
